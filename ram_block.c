...
#include <linux/genhd.h> //  basic block driver framework
...
/* 
  The internal structure representation of my Device
 */
static struct rb_device
{
	/* Size is the size of the device in sectors */
	unsigned int size;
	/*  exclusive access to my request queue */
	spinlock_t lock;
	/* my request queue */
	struct request_queue *rb_queue;
	/* This is how kernel represent an individual disk device */
...
/* 
  below are the file operations that performed on the ram block device
 */
...
/* 
  Below is registration and initialization section of the ram block device
  driver
 */
static int __init rb_init(void)
{
	...
	printk(KERN_INFO "Ram Block driver initialised (%d sectors; %d bytes)\n",
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("praveen hooda");
MODULE_DESCRIPTION("disc on ram");
